...
namespace e_commerce_app_api.Repositories.Repository
{
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<AccountDetailDTO> GetAccountById(int id) => await AccountDAO.Instance.GetAccountById(id);
        public async Task<List<AccountDTO>> GetAccounts() => mapper.Map<List<AccountDTO>>(await AccountDAO.Instance.GetAccounts());
        public async Task<AccountDTO> UpdateAccount(AccountRequestUpdateDTO account, int id)
        {
            return mapper.Map<AccountDTO>(await AccountDAO.Instance.UpdateAccount(account, id));
        }
    }
}
